...
func TestServerCluster(t *testing.T) {
	...
	t.Run("peer-remove standalone server", func(t *testing.T) {
		withJSServer(t, func(t *testing.T, srv *server.Server, nc *nats.Conn, mgr *jsm.Manager) error {
			...
			} else if !strings.Contains(err.Error(), "did not receive a response from the meta leader, ensure the account used has system privileges and appropriate permissions") {
				t.Errorf("expected 'did not receive a response from the meta leader, ensure the account used has system privileges and appropriate permissions error, got: %v", err)
			}
			return nil
		})
	})
